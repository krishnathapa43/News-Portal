...
<?php
$postid=intval($_GET['pid']);
$query=mysqli_query($con,"select posts.id as postid,posts.PostImage,posts.PostTitle as title,posts.PostDetails,category.CategoryName as category,category.id as catid,subcategory.SubCategoryId as subcatid,subcategory.Subcategory as subcategory from posts left join category on category.id=posts.CategoryId left join subcategory on subcategory.SubCategoryId=posts.SubCategoryId where posts.id='$postid' and posts.Is_Active=1 ");
